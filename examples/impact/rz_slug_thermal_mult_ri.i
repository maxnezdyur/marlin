# REDUCED-INTEGRATION variant: single quadrature point per element (4x fewer
# constitutive evaluations) with batched Flanagan-Belytschko hourglass control
# instead of F-bar (one-point elements do not lock volumetrically).
# Axisymmetric (RZ) Taylor impact of shot CuH04_235.9 through the NEML2
# nodal-force path: MULTIPLICATIVE finite-strain, thermally-coupled
# (adiabatic) Johnson-Cook, explicit central difference with
# manage_state_advance. Compression-safe successor of the additive
# rz_slug_thermal_large_def.i (SVK collapse at the foot).
#
...
[NEML2]
  input = 'johnson_cook_neml2_mult_thermal_cal.i'
  [all]
    executor_name = 'neml2'
    model = 'model'
    verbose = false
...
[UserObjects]
  ...
  [hourglass]
    ...
    executor = 'neml2'
    displacements = 'disp_x disp_y'
    # classic FB coefficient ~0.05 with the physical shear modulus
    # mu = E/(2(1+nu)) = 117e9/2.68
    penalty = 0.05
